...
search_query = st.sidebar.text_input(f"Enter {search_option} (partial or full)")

if search_option == "Anime Title" and search_query:
    results = data[data["title"].str.contains(search_query, case=False, na=False)] if "title" in data.columns else pd.DataFrame()
    st.subheader(f"Results for Anime Title containing: '{search_query}'")
    st.dataframe(results[[c for c in ["title", "score", "members", "year", "studios", "genres"] if c in results.columns]])

if search_option == "Studio" and search_query:
    if "studios" in data.columns:
        results = data[data["studios"].fillna("").str.contains(search_query, case=False, na=False)]
    else:
        results = pd.DataFrame()
    st.subheader(f"Anime produced by Studio containing: '{search_query}'")
    st.dataframe(results[[c for c in ["title", "score", "members", "year", "studios", "genres"] if c in results.columns]])

if search_option == "Genre" and search_query:
    results = data[data["genres_list"].apply(lambda gl: any(search_query.lower() in g.lower() for g in gl))]
    st.subheader(f"Anime in Genre containing: '{search_query}'")
    st.dataframe(results[[c for c in ["title", "score", "members", "year", "studios", "genres"] if c in results.columns]])
